// Consts
const apikey = "93f05d46b0f5f9b2e7dfe9393a21dcf0";
...
// Capture the search input and search icon click event
...
function searchMovies() {
    ...
    if (movieName === "") return;  // If no movie name is entered, do nothing

    // Call your movie recommendation function (assuming a function like `getMovieRecommendations` exists)
    getMovieRecommendations(movieName);
}

function getMovieRecommendations(movieName) {
...
function displayRecommendations(recommendedMovies) {
    ...
    const banner = document.getElementById('banner-section'); // Get the banner section

    // Show the overlay and hide the banner
    ...
    recommendedMovies.forEach(movie => {
        ...
        posterImg.src = movie.poster_path ? `${imgPath}${movie.poster_path}` : 'default-poster.jpg';
...
// Add an event to hide the overlay and show the banner again
document.getElementById('overlay').addEventListener('click', function() {
    this.style.display = 'none'; // Hide the overlay
    const banner = document.getElementById('banner-section');
    banner.classList.remove('hide-banner'); // Show the banner again
    document.getElementById('recommendations-container').innerHTML = ''; // Clear recommendations
});
...
document.getElementById('home-nav').addEventListener('click', function() {
    ...
    // Optionally reset input fields, if any
